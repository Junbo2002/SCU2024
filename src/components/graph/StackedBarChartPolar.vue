<template>
    <div
      ref="StackedBarChartPolar"
      id="stackBarChartPolar"
      :style="{ height: '70vh'}"
    ></div>
  ...
  export default {
    ...
    onMouseUp() {
      ...
      var p = [Math.round(this.points[0]), Math.round(this.points[1]), Math.round(this.points[2])];
      var value = p.sort(function(a,b){
            if (a>b) {
               return 1;
           }else if(a<b){
               return -1
      ...
      console.log(value);
      this.option["radiusAxis"]["data"] = [value[2] +'+' , value[1]+'-'+value[2] , value[0]+'-'+value[1] , value[0]+'-']
