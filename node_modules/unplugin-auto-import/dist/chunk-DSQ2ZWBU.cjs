...
var _chunkOQWD4OKRcjs = require('./chunk-OQWD4OKR.cjs');
...
function normalizeImport(info, name) {
  ...
  return _chunkOQWD4OKRcjs.__spreadValues.call(void 0, {
    name,
    as: name
  }, info);
}
...
`;
  ...
  const componentCustomPropertiesReg = new RegExp("interface\\s+ComponentCustomProperties\\s*{(.*?)[\\n\\r]}", "gs");
...
async function flattenImports(map) {
  const promises = await Promise.all(_utils.toArray.call(void 0, map).map(async (definition) => {
    if (typeof definition === "string") {
      if (!_chunkOQWD4OKRcjs.presets[definition])
        throw new Error(`[auto-import] preset ${definition} not found`);
      const preset = _chunkOQWD4OKRcjs.presets[definition];
